...
namespace org.neuroph.samples.ionosphere
{
	 ...
	 */
	public class IonosphereSample : LearningEventListener
	{
	...
	//        }
		...
		public virtual void testNeuralNetwork(NeuralNetwork neuralNet, DataSet testSet)
		{
			...
			Console.WriteLine("Prediction for 'Good' => (Correct/total): " + this.correct[0] + "/" + count[0] + "(" + formatDecimalNumber(percentM) + "%). ");

			double percentB = (double) this.correct[1] * 100.0 / (double) this.count[1];
			Console.WriteLine("Prediction for 'Bad' => (Correct/total): " + this.correct[1] + "/" + count[1] + "(" + formatDecimalNumber(percentB) + "%). ");
