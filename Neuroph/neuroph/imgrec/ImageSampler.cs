...
namespace org.neuroph.imgrec
{
	...
	using ImageType = org.neuroph.imgrec.image.ImageType;
	...
	//	}
		...
		public static Image downSampleImage(Dimension samplingResolution, Image bigImage, int returnImageType)
		{
			int inspectX;
			int inspectY;
			int numberOfSamplesAcross = samplingResolution.Width;
			...
			if (bigImage.Width <= numberOfSamplesAcross || bigImage.Height <= numberOfSamplesDown)
			{
				return bigImage;
			}

			Image image = ImageFactory.createImage(new int?(numberOfSamplesAcross), new int?(numberOfSamplesDown), new int?(returnImageType));

			double samplingIncrementX = bigImage.Width / (samplingResolution.Width - 1);
			double samplingIncrementY = bigImage.Height / (samplingResolution.Height - 1);
			...
			for (int y = 0; y < numberOfSamplesDown; y++)
			{
				for (int x = 0; x < numberOfSamplesAcross; x++)
				{
					...
					if (inspectX >= bigImage.Width)
					{
						inspectX = bigImage.Width - 1;
					}
					if (inspectY >= bigImage.Height)
					{
						inspectY = bigImage.Height - 1;
					}
