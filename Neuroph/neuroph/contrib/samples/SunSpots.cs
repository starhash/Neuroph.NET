...
namespace org.neuroph.contrib.samples
{
	...
	using Neuroph = org.neuroph.util.Neuroph;
	...
	public class SunSpots : LearningEventListener
	{
		...
		/// <summary>
		/// Normalize the sunspots. </summary>
		/// <param name="lo"> Low range for normalization. </param>
		/// <param name="hi"> High range for normalization. </param>
		public virtual void normalizeSunspots(double lo, double hi)
		{
		...
		/// <summary>
		/// Generate the training data for the training sunspot years. </summary>
		/// <returns> The training data. </returns>
		public virtual DataSet generateTrainingData()
		{
		...
		/// <summary>
		/// Predict sunspots. </summary>
		/// <param name="network"> Neural network to use. </param>
		public virtual void predict(NeuralNetwork network)
		{
			NumberFormat f = NumberFormat.NumberInstance;
			f.MaximumFractionDigits = 4;
			f.MinimumFractionDigits = 4;

			System.Console.WriteLine("Year\tActual\tPredict\tClosed Loop Predict");
			...
			for (int year = EVALUATE_START; year < EVALUATE_END; year++)
			{
				...
				// display
				System.Console.WriteLine((STARTING_YEAR + year) + "\t" + f.format(this.normalizedSunspots[year]) + "\t" + f.format(prediction) + "\t" + f.format(closedLoopPrediction));
		...
		public virtual void run()
		{
			...
			network.LearningRule.addListener(this);
		...
		{
			SupervisedLearning rule = (SupervisedLearning)@event.Source;
