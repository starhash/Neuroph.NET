...
namespace org.neuroph.ocr
{
	...
	public class CharExtractor
	{
		...
		private bool endOfImage; //end of picture
		private bool endOfRow; //end of current reading row

		/// <summary>
		/// Creates new char extractor with soecified text image
		/// TODO: add background and foregrounf color </summary>
		/// <param name="imageWithChars"> - image with text </param>
		public CharExtractor(BufferedImage imageWithChars)
		{
			this.imageWithChars = imageWithChars;
		}

		public virtual BufferedImage ImageWithChars
		{
			set
			{
				this.imageWithChars = value;
			}
		}
		...
		/// When it finds line whith all white pixels, it sets cropBottomY and returns true </summary>
		/// <returns> - returns true when cropBottomY value is set, false otherwise </returns>
		private bool findCropBottomY()
		{
			for (int y = cropTopY + 1; y < imageWithChars.Height; y++) // scan image from top to bottom
			{
				int whitePixCounter = 0; //counter of white pixels in a row
				for (int x = cropLeftX; x < imageWithChars.Width; x++) // scan all pixels to right starting from left crop position
				{
				...
				if (whitePixCounter == imageWithChars.Width) // if we have reached end of line counting white pixels (x pos)
				{
				...
				if (y == imageWithChars.Height - 1) // if we have reached end of image
				{
		...
		/// When black pixel is found it sets cropLeftX and returns true. It should set cropLeftX to the next letter in a row. </summary>
		/// <returns> - return true when true when black pixel is found and cropLeftX is changed, false otherwise </returns>
		private bool findCropLeftX()
		{
			int whitePixCounter = 0; // white pixel counter between the letters
			for (int x = cropRightX; x < imageWithChars.Width; x++) // start from previous righ crop position (previous letter), and scan following pixels to the right
			{
		...
		/// TODO: resiti broblem tolerancije n aboju bacgrounda kada ima prelaz... </summary>
		/// <returns> - return true  when x2 value is changed and false when x2 value is not changed </returns>
		private bool findCropRightX()
		{
			for (int x = cropLeftX + 1; x < imageWithChars.Width; x++) // start from current cropLeftX position and scan pixels to the right
			{
				...
				if (x == imageWithChars.Width - 1) // if we have reached end of row with x position
				{
	...
	//        endOfRow = true;                                                    // da li ovo treba , nije ga bilo?
		...
		/// <summary>
		/// Creates HashMap with characters as keys and BufferedImages as values </summary>
		/// <param name="Dimensions"> to which output char images has to be resized to </param>
		/// <param name="list"> of letters which are names of images </param>
		/// <returns> HashMap with characters as keys and char images as values
		/// This method returns HashMap with characters as keys and char images as values </returns>
		public virtual Dictionary<string, BufferedImage> extractCharImagesToLearn(BufferedImage imageWithText, List<string> chars, Dimension dim)
		{
			this.imageWithChars = imageWithText;

			Dictionary<string, BufferedImage> charImages = new Dictionary<string, BufferedImage>();
			...
			while (endOfImage == false) // until the end of the image is reached do the following
			{
				...
				bool foundTop = findCropTopY(); // find top coordinate for crop
				bool foundBottom = false; // reset foundBottom flag on each new text row start, after the top coordinate is found
				if (foundTop == true) // make sure we have top position and proceed. Otherwise we have reached end of image
				{
					...
					if (foundBottom == true) // make sure we have bottom coordinateand  proceed. At this point we have top and bottom crop coordinates.
					{
						...
						{
							bool foundLeft = false; // at  begining of the row set left and right crop flags to false
	...
	//                                }    else 
									...
									charImage = ImageUtilities.resizeImage(charImage, dim.Width, dim.Height);
									charImages[chars[i]] = charImage; // add char image to collection which will be returned
